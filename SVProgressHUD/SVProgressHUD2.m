...
#error SVProgressHUD2 is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "SVProgressHUD2.h"
...
@property (nonatomic, strong) NSOperationQueue *presentationQueue;

@end

@implementation SVProgressHUD2 {
    BOOL _isInitializing;
}

+ (SVProgressHUD2*)sharedView {
    static dispatch_once_t once;
    
    static SVProgressHUD2 *sharedView;
...
+ (void)popActivity {
    [[SVProgressHUD2 sharedView].presentationQueue addOperationWithBlock:^{
        [SVProgressHUD2 sharedView].presentationQueue.suspended = YES;
        
        dispatch_async(dispatch_get_main_queue(), ^{
            if([SVProgressHUD2 sharedView].activityCount > 0) {
                [SVProgressHUD2 sharedView].activityCount--;
            }
            
            if([SVProgressHUD2 sharedView].activityCount == 0) {
                [[SVProgressHUD2 sharedView] dismissWithDelay:0.0 completion:^{
                    [SVProgressHUD2 sharedView].presentationQueue.suspended = NO;
                }];
            } else {
                [SVProgressHUD2 sharedView].presentationQueue.suspended = NO;
            }
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        NSBundle *bundle = [NSBundle bundleForClass:[SVProgressHUD2 class]];
        ...
        _presentationQueue = [[NSOperationQueue alloc] init];
        _presentationQueue.maxConcurrentOperationCount = 1;
...
#else
            // If SVProgressHUD2 is used inside an app extension add it to the given view
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak SVProgressHUD2 *weakSelf = self;
    
    [[SVProgressHUD2 sharedView].presentationQueue addOperationWithBlock:^{
        [SVProgressHUD2 sharedView].presentationQueue.suspended = YES;
        
        dispatch_async(dispatch_get_main_queue(), ^{
            __strong SVProgressHUD2 *strongSelf = weakSelf;
            if(strongSelf){
                ...
                [strongSelf fadeInWithCompletion:^{
                    [SVProgressHUD2 sharedView].presentationQueue.suspended = NO;
                }];
...
- (void)fadeInWithCompletion:(void (^ __nonnull)(void))completion {
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(SVProgressHUDDismissCompletion)completion {
    ...
    __block void (^completionBlock)(void) = ^{
        ...
        if(strongSelf && strongSelf.backgroundView.alpha == 0.0f){
